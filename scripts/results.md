...
## 16. Pooled Model + Euclidean Alignment (EA)

**Setup:** Identical to Section 14 (K=10K/subject, T_min=500ms, N_opt=20), with one addition: before pooling, each subject's training windows are spatially whitened to make their mean covariance equal to the identity matrix. The whitening matrix is computed from training windows only (no leakage), then applied to both training and test windows for that subject.

**Euclidean Alignment formula:**

For subject *s*, compute mean spatial covariance from all N_s training windows:

$$\bar{R}_s = \frac{1}{N_s} \sum_i x_i^\top x_i / T$$

Whitening matrix: $W_s = \bar{R}_s^{-1/2}$. Apply: $\tilde{x}_i = x_i W_s^\top$.

This re-centres each subject's covariance distribution to the identity, removing inter-subject amplitude and channel-correlation differences before pooling. StandardScaler inside the SVM pipeline applies on top.

**Optuna best:** AUC=0.8374, T=87 samples (1403 ms), C=2.509, γ=4.21×10⁻²  
**Val AUC (20% holdout):** 0.9601 *(heavily inflated — pool val set shares subjects with training)*

### Per-subject test results (trials 8–9)

| Subject | EA AUC | EA Sens | EA EvtSens | EA FA/min | Baseline AUC | Baseline EvtSens |
|---|---|---|---|---|---|---|
| 0 | 0.507 | 0.236 | 0.491 | 20.4 | 0.528 | 0.527 |
| 1 | **0.641** | 0.297 | **0.596** | 77.4 | 0.613 | 0.479 |
| 2 | 0.575 | 0.284 | 0.562 | 23.7 | 0.541 | 0.750 |
| 3 | 0.512 | 0.069 | 0.250 | 9.3 | **0.696** | **0.875** |
| 4 | 0.502 | 0.352 | **0.623** | 26.0 | 0.526 | 0.344 |
| 5 | **0.659** | **0.463** | 0.476 | 15.2 | 0.617 | 0.714 |
| 6 | 0.392 | 0.178 | 0.250 | 13.4 | 0.528 | 0.167 |
| 8 | 0.527 | 0.109 | 0.000 | 10.8 | 0.391 | 0.000 |
| 9 | 0.512 | 0.217 | 0.354 | 25.7 | 0.477 | 0.446 |
| **Mean** | **0.536** | **0.245** | **0.400** | **24.6** | **0.546** | **0.478** |

*(Baseline = K=10K T≥500ms, no EA, from Section 14)*

### Summary vs baseline

| Metric | Baseline (no EA) | + EA | Delta |
|---|---|---|---|
| Mean test AUC | 0.546 | 0.536 | −0.010 |
| Mean test Sens | 0.259 | 0.245 | −0.014 |
| Mean test EvtSens | 0.478 | 0.400 | −0.078 |

### Key observations

1. **EA did not improve generalisation** — mean AUC and EvtSens both dropped slightly. This is contrary to the typical finding in motor imagery BCI literature where EA yields +3–8 pp.
2. **Val AUC inflated to 0.96** (vs 0.767 baseline). EA removes inter-subject variance from the pool, so the held-out 20% of the pool (same subjects, different windows) looks easier — but this does not transfer to unseen trials 8–9.
3. **Mixed per-subject effects.** EA helps subjects 1, 4, 5, 8, 9 (AUC gains of 0.03–0.14) but hurts subjects 3 and 6 severely (−0.18 and −0.14 AUC). Subject 3 was the strongest pooled subject (0.696) and EA destroyed its signal.
4. **Likely reason EA fails here:** CAR is already applied before EA. CAR partially whitens the spatial covariance (it projects out the common mode), leaving a rank-deficient 16×16 covariance. EA's `R^{-1/2}` then amplifies the small remaining singular values, potentially distorting rather than aligning the signal. With only 16 channels and high-amplitude artefact subjects (6, 8) in the pool, the mean covariance is not a reliable normalisation target.
5. **Alternative alignment strategies to try:** per-trial z-score of the raw EEG before windowing (simpler, more robust), or excluding artefact subjects (6, 8) from the pool before computing EA.

*Script:* `scripts/svm_pooled_ea.py`
